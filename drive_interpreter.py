...
class CheesyDriveHelper():
    ...
    def DriveSignal(self, throttle:float, wheel:float, quick_turn:bool, high_gear:bool):
        ...
        wheel = _handle_deadband(wheel, wheel_deadband)
        throttle = _handle_deadband(throttle, throttle_deadband)
        ...
        neg_inertiaScalar = 0
        if high_gear:
            neg_inertiaScalar = high_neg_inertia_scalar
            sensitivity = high_sensitivity
        else:
            if wheel * neg_inertia > 0:
                neg_inertiaScalar = low_neg_inertia_turn_scalar
            else:
                if abs(wheel) > low_neg_inertia_threshold:
                    neg_inertiaScalar = low_neg_inertia_far_scalar
                else:
                    neg_inertiaScalar = low_neg_inertia_close_scalar
            sensitivity = low_sensitivity

        neg_inertiaPower = neg_inertia * neg_inertiaScalar
        neg_inertia_accumulator += neg_inertiaPower
        ...
        if quick_turn:
            if abs(linear_power) < quick_stop_deadband:
                alpha = quick_stop_weight
                self.quick_stop_accumulator = (1- alpha) * self.quick_stop_accumulator + alpha * _limit(wheel, 1) * quick_stop_scalar

            over_power = 1
            angular_power = wheel
        else:
            over_power = 0
            angular_power = abs(throttle) * wheel * sensitivity -quick_stop_accumulator
